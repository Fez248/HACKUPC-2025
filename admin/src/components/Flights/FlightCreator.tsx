...
interface FlightCreatorProps {
  onFlightCreated: (flight: Flight) => void;
}

const FlightCreator: React.FC<FlightCreatorProps> = ({ onFlightCreated }) => {
  const initialFormData: Omit<Flight, 'id'> = {
    flightNumber: '',
    originFull: '',
    originShort: '',
    departureTime: '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessage({ 
        type: 'success', 
        text: response.message || 'Flight created successfully'
      });
      
      if (response.flight) {
        onFlightCreated(response.flight);
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-5 mb-6">
      <h3 className="text-xl font-semibold mb-4">Create New Flight</h3>
